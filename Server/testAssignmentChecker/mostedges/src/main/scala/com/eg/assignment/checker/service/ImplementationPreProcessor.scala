...
class ImplementationPreProcessor(ancestor: String) extends PreProcessor[Either[Exception, MostEdges]] with LazyLogging {
  override val getProcessResult: Either[Exception, MostEdges] = {
    tryAllExceptions(ClassFinderHelper.findSubclasses(ancestor) match {
      ...
      case classInfo :: Nil => ClassInstanceHelper.newInstance[MostEdges](classInfo.name)
