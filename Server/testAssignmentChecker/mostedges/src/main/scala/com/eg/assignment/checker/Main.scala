...
object Main extends App with LazyLogging {
  (for {
    ...
    ancestor = "com.eg.assignment.MostEdges"
  ...
  private def findImplementation(ancestor: String): Either[Exception, MostEdges] =
    ImplementationPreProcessor(ancestor).getProcessResult

  private def runTests(instance: MostEdges): Either[Exception, List[TestSuiteResult]] =
